...
#include "ExpertChargerRectangle.h"
#include <vector>
#include "../Formes/Rectangle.h"

Forme* ExpertChargerRectangle::charger2(const std::string& ligne) const {
    ...
    if (points.size() == (size_t)4) {
        //on regarde si une couleur a été définie, sinon on applique la couleur noire
        if (i >> color) return new Formes::Rectangle(points[0].getX(),points[0].getY(),points[1].getX(),points[1].getY(),points[2].getX(),points[2].getY(),points[3].getX(),points[3].getY(),color);
        else return new Formes::Rectangle(points[0].getX(),points[0].getY(),points[1].getX(),points[1].getY(),points[2].getX(),points[2].getY(),points[3].getX(),points[3].getY(),color);
    }
